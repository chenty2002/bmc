...
class BMC:
    ...
    def get_map(self, idx):
        curr_map = [(self.literals[i], self.vardef(str(self.literals[i]) + "_" + str(idx))) for i in
                    range(len(self.literals))]
        next_map = [(self.primes[i], self.vardef(str(self.literals[i]) + "_" + str(idx + 1))) for i in
                    range(len(self.literals))]
    ...
    def unroll(self):
        ...
        # Constrain the input variables as well input_map = [(self.primary_inputs[i], self.vardef(str(
        # self.primary_inputs[i])+"_"+str(idx))) for i in range(len(self.primary_inputs))] self.slv.add(substitute(
        # And([pi == pi_prime for pi, pi_prime in self.inp_map]), input_map))
        self.cnt += 1
